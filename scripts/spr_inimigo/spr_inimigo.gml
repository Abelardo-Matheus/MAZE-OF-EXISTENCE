...
function scr_recalculate_path() {
    ...
    var player_x = obj_player.x div global.cell_size;
    var player_y = obj_player.y div global.cell_size;
    ...
    if (ds_list_size(path) > 0) {
        ...
        target_x = point[0] * global.cell_size;
        target_y = point[1] * global.cell_size;
    }
}
...
function create_random_enemy_folow() {
    ...
    if (lab != noone) {
        ...
        instance_create_layer(enemy_x * lab.cell_size, enemy_y * lab.cell_size, "Top_layer", obj_enemy_folow);
    }
}
...
function scr_enemy_random_move() {
    ...
    while (!valid_direction_found && ds_list_size(directions) > 0) {
        ...
        var new_x = (x div global.cell_size) + directio[0];
        var new_y = (y div global.cell_size) + directio[1];

        // Verifica se a nova posição não é uma parede
        if (ds_grid_get(global.maze, new_x, new_y) == 1) { // 1 significa chão
            target_x = new_x * global.cell_size;
            target_y = new_y * global.cell_size;
            is_moving = true;
